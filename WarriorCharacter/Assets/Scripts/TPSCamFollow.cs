...
public class TPSCamFollow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Cam.position = Vector3.Lerp(Cam.position,camHit.point+(transform.position-Cam.position).normalized*1,20*Time.deltaTime);
        }
        else
        {
            Cam.localPosition = Vector3.Lerp(Cam.localPosition,CamOffset, 20 * Time.deltaTime);
        }
    }
}
